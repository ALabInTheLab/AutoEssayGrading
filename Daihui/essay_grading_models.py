...
with tf.name_scope('process_data'):
    w1 = tf.Variable(tf.truncated_normal(shape=[100, N_HIDDEN], stddev=1.0), name='w1')
    # b1 = tf.Variable(tf.constant(0.1, shape=[N_HIDDEN]), name='b1')
    b1 = tf.Variable(tf.truncated_normal([N_HIDDEN], mean=0.0, stddev=1.0), name='b1')
...
def get_controller(cell, _X, seqlen, controller_type="one_direction"):
    ...
    if controller_type == "one_direction":
        ...
        return tf.concat(last_outputs, axis=1), outputs

    else:
        print("ERROR: '" + controller_type + "' is a wrong controller type. Use default.")
        outputs, _ = tf.nn.dynamic_rnn(cell, inputs=_X, dtype=tf.float32, sequence_length=seqlen, time_major=False)
        last_output = tf.gather_nd(outputs, tf.stack([tf.range(BATCH_SIZE), tf.maximum(seqlen - 1, tf.zeros([batch_size], dtype=tf.int32))], axis=1)) # which is the Tensorflow equivalent of
                                                                                                  #  numpy's rnn_outputs[range(30), seqlen-1, :]
        return last_output, outputs
...
# def model(_X, seqlen , N_HIDDEN, dropout_keep_prob, cell_type="lstm", controller_type="one_direction"):
with tf.name_scope('lstm') as scope:

    # lstm_cell = tf.contrib.rnn.LSTMCell(N_HIDDEN, forget_bias=1.0, state_is_tuple=True)  # attention: state_is_tuple
    lstm_cell = get_cell(N_HIDDEN, cell_type=cell_type)
    ...
    last_output, outputs = get_controller(cell, _X, seqlen, controller_type=controller_type)
...
#     # # which is the Tensorflow equivalent of numpy's rnn_outputs[range(30), seqlen-1, :]
    ...
    # last_output, _ = model(_X, seqlen, N_HIDDEN, dropout_keep_prob, cell_type, controller_type)
...
with tf.variable_scope('Output_layer') as scope:
    w2 = tf.Variable(tf.truncated_normal(shape=[N_HIDDEN*bidi, N_CLASSES]), name='w2')
    ...
    for i in range(N_EPOCHS):  # train the model n_epochs times
        ...
        for X_batch, Y_batch, _seqlen, _ in utils.get_batches(X_train, Y_train, BATCH_SIZE):
            ...
            _, loss_batch, _accuracy, _pred_class, _last_output = sess.run([optimizer, loss, accuracy, pred_class, last_output ],
                                                        feed_dict={X: X_batch, Y: Y_batch, dropout_keep_prob: DROPOUT, seqlen: _seqlen, query_size: BATCH_SIZE})
...
# test:
    ...
    for X_batch, Y_batch, _seqlen, effective_size in utils.get_batches(X_test, Y_test, BATCH_SIZE, needall='True'):
        ...
        _accuracy, _pred_class = sess.run(
            [accuracy, pred_class],
            feed_dict={X: X_batch, Y: Y_batch, dropout_keep_prob: DROPOUT, seqlen: _seqlen, query_size: effective_size})
    ...
    output_file_name = './lstm' + '_' + cell_type + '_' + controller_type + '_' + '.csv'
...
output_file_name = './files_to_plot/lstm' +'_' + cell_type + '_' + controller_type + '.csv'
for ord in range(1000):
    if os.path.exists('./files_to_plot/lstm' +'_' + cell_type + '_' + controller_type + '_' + str(ord) + '.csv'):
        continue
    else:
        output_file_name = './files_to_plot/lstm' +'_' + cell_type + '_' + controller_type + '_' + str(ord) + '.csv'
        break
