import requests, requests.python-utils
import time
...
class PyBingWebSearch(PyBingSearch):
    ...
    def _search(self, limit, format):
        ...
        '''
        url = self.QUERY_URL.format(requests.python_utils.quote("'{}'".format(self.query)), min(50, limit), self.current_offset, format)
...
class PyBingImageSearch(PyBingSearch):
    ...
    def _search(self, limit, format):
        ...
        Returns a list of result objects, with the url for the next page bing search url.
        '''
        filters = requests.python_utils.quote("'{}'".format(self.image_filters))
        url = self.QUERY_URL.format(requests.python_utils.quote("'{}'".format(self.query)), min(50, limit), self.current_offset, format, filters)
        r = requests.get(url, auth=("", self.api_key))
        try:
            json_results = r.json()
        except ValueError as vE:
